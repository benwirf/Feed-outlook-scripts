...
from qgis.core import (QgsField, QgsFeature, QgsProcessing,
...
class TotalGrowthSummary(QgsProcessingAlgorithm):
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        total_growth_raster = processing.run('native:cellstatistics', cell_sum_params, context=context, feedback=feedback, is_child_algorithm=True)['OUTPUT']
        ...
        for district_name in district_names:
            ...
            mask_params = {'INPUT':total_growth_raster,
                'MASK':temp_districts,
                'SOURCE_CRS':None,
                'TARGET_CRS':None,
                'NODATA':None,
            ...
            clipped = processing.run("gdal:cliprasterbymasklayer", mask_params, context=context, feedback=feedback, is_child_algorithm=True)['OUTPUT']
            raster1 = gdalnumeric.LoadFile(clipped)
            
########****************UP TO HERE***********************************###########
#TODO: Call total_growth counts, passing each clipped raster and region retrieved from regions
# dict. Add a feature with counts etc. for each district to temp layer then export it as spreadsheet
 
        return {'Folder': monthly_growth_folder}
